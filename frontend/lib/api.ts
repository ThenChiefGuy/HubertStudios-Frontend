...
// Settings returned by the Worker. These control global hash enforcement and
// provide a read-only summary of the email configuration. There is currently
// only one editable setting: `globalRequireHash`. Email provider/from are
// informational only and pulled from environment variables on the Worker.
...
/**
 * Fetch the current global settings from the Worker. Requires an admin session.
 */
...
/**
 * Update global settings. Currently only `globalRequireHash` can be toggled.
 * Returns the updated settings object.
 */
export async function updateSettings(input: { globalRequireHash: boolean }) {
  const data = await request<{ ok: true; settings: ApiSettings }>('/api/admin/settings', {
    method: 'PUT',
    body: input,
  })
